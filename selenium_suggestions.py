...
def get_suggested_accounts(wait_func):
    _all_buttons = wait_func.until(EC.presence_of_all_elements_located((By.XPATH, "//div[@role='button']//div//*[local-name()='svg']")))
...
def search_suggested_accounts(browser):
    # Screen size
    ...
    for idx, profile in enumerate(profiles_key_word):
        ...
        # Search for profile
        enter_search_text(wait, profile)
        # send input keys to input field
        profile_href = "/" + profile + "/"
        profile_card = wait.until(EC.presence_of_element_located((By.XPATH, f"//a[@href='{profile_href}']")))  # Async wait for search result
        mouse_loc = profile_card.location  # for mouse movement
        profile_link = profile_card.get_attribute('href')
        print(f'Profile_card: {profile_link}')
        ACTION.move_to_element(profile_card)
        ACTION.click()
        ACTION.perform()
        time.sleep(2)
        ...
        while result is None:
            try:
                # get all link (TODO all links find profile posts IDS too)
        ...
        #if suggested_idx == 0:
        #    print(f'Probably not valid: {profile}')
        #    # Clear search
...
BASE_FOLDER = "db\\demo\\"
KEY_WORD_SEARCH_FILE = "key_word_search_20230808_11_32_29.csv"
# ...................
# Parameters
...
for i in range(MAX_TRIES):
    ...
    try:
        ...
        driver.get(BASE_URL)
        # Find suggested
        search_suggested_accounts(driver)
    except:
